...
y_all = np.load("../temp/y_p.npy")#np.concatenate((np.sin(theta),np.cos(theta)),axis=1)
#energy=np.load("/home/duyang/meng/txt/y_7.npy")[:,6]
anglex_train = np.sin(y_all[:,0])*np.cos(y_all[:,1])
angley_train = np.sin(y_all[:,0])*np.sin(y_all[:,1])
anglez_train = np.cos(y_all[:,0])
y_all = np.stack((anglex_train,angley_train,anglez_train), axis=-1)
...
class SphereNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 =SphereConv2d(4, 16,kernel_size=(7, 7),stride=1)
        self.pool1 = SphereMaxPool2d(stride=2)
        self.conv2 = SphereConv2d(32, 32, kernel_size=(7, 7),stride=1)
        self.pool2 = SphereMaxPool2d(stride=2)
        self.conv3 = SphereConv2d(16,32, kernel_size=(7, 7),stride=1)
        self.conv4 = SphereConv2d(32,64, kernel_size=(5, 5),stride=1)
        self.conv5 = SphereConv2d(64,64, kernel_size=(5, 5),stride=2)
        self.conv8 = SphereConv2d(32,16, kernel_size=(5,5),stride=1)
        self.conv6 = SphereConv2d(64,32, kernel_size=(5, 5),stride=1)
        self.conv7 = SphereConv2d(64,128, kernel_size=(1, 1),stride=2)
        self.dropout3=nn.Dropout(0.25)
        self.batch_norm1 = nn.BatchNorm2d(32)
        self.batch_norm2 = nn.BatchNorm2d(8)
        self.batch_norm3 = nn.BatchNorm2d(64)
        self.batch_norm4 = nn.BatchNorm2d(16)
        self.conv2d=nn.Conv2d(32,16,3 )
        self.fc1 = nn.Linear(1040, 128)
        self.batchnorm4 = nn.BatchNorm1d(1)
        self.fc2 = nn.Linear(128, 64)
        self.fc3 = nn.Linear(520,3)
        self.conv2d1 = nn.Conv2d(16,8,kernel_size=(4,4))
        self.m =nn.AvgPool2d((6,7))
    def residual_block(self,x,n_in):
        batchnorm = nn.BatchNorm2d(n_in)
        conv = SphereConv2d(n_in, n_in,kernel_size=(3, 3),stride=1)
        out = F.relu(batchnorm(conv(x)))
        out = conv(out)
        return F.relu(batchnorm(x+out))
    def forward(self, x):
        x = self.conv1(x)
        x = F.relu(self.batch_norm1(self.conv3(x)))
        x = self.pool1(x)  #63 126 64
        x = self.conv2(x)
        x = F.relu(self.batch_norm3(self.conv4(x)))
        x = self.pool1(x) # 32*63*64
        x = F.relu(self.batch_norm3(self.conv5(x)))
        x = F.relu(self.batch_norm1(self.conv6(x)))
        x = self.pool1(x) #16 32 32
        x=  F.relu(self.batch_norm1(self.conv2(x)))
        x =  F.relu(self.batch_norm4(self.conv8(x)))
        x = self.pool1(x) #second block 8*16*16
        x=  F.relu(self.batch_norm2(self.conv2d1(x)))  #5*13*8
        #x = F.relu(self.batch_norm4(self.conv2d(x))) #5*13*16
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            for idx in range(output.size(0)):
                v_pre.append(output[idx].detach().cpu().numpy())
